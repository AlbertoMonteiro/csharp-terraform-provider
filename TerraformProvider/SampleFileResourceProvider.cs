...
public class SampleFileResourceProvider : IResourceProvider<SampleFileResource>
{
    ...
    public SampleFileResourceProvider(SampleConfigurator configurator) => _configurator = configurator;

    public Task<SampleFileResource> PlanAsync(SampleFileResource? prior, SampleFileResource proposed) => Task.FromResult(proposed);
    ...
    private string BuildContent(string content)
    {
        ...
        var unused1 = sb.AppendLine(header);
        var unused = sb.AppendLine(content);
        _ = sb.AppendLine("FROM DOTNET");
        return sb.ToString();
    }
}